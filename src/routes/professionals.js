...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    const { full_name, email, role, credential_number } = req.body;
    const { rows: [pro] } = await pool.query(
      `INSERT INTO professionals (clerk_user_id, email, full_name, role, credential_number)
       VALUES ($1,$2,$3,$4,$5) RETURNING *`,
      [userId, email, full_name, role || 'rbt', credential_number || null]
    );
    res.json(pro);
  } catch (err) {
    res.status(500).json({ error: err.message });
